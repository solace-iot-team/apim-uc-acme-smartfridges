...
class Config {
  ...
  constructor() {
    ...
    this.#server = {
      port: getEnvVarAsNumber('ASFAB_SERVER_PORT', Constants.DEFAULT_SERVER_PORT),
      requestLimit: getEnvVarAsString('ASFAB_SERVER_REQUEST_LIMIT', Constants.DEFAULT_SERVER_REQUEST_LIMIT),
      corsOrigin: getEnvVarAsString('ASFAB_SERVER_CORS_ORIGIN', Constants.DEFAULT_SERVER_CORS_ORIGIN).split(/\s*,\s*/),
    }

    this.#broker = {
      endpoint: getMandatoryEnvVarAsString('ASFAB_BROKER_ENDPOINT'),
      username: getMandatoryEnvVarAsString('ASFAB_BROKER_USERNAME'),
      password: getMandatoryEnvVarAsString('ASFAB_BROKER_PASSWORD'),
    }
  }
